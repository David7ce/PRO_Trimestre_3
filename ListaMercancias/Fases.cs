...
public class Fases
{
    ...
    public static List<Mercancia> A_CargarDatosConsola(out Decimal precioKilo)
    {
        ...
        // Con lectura anticipada, recomendado por el profe
        Console.Write("Escribe el destino: ");
        ...
        while (pais != null && pais != "0")
        {
            ...
            // Volvemos a pedir el destino
            Console.Write("Escribe el destino: ");
            pais = Console.ReadLine();
        }

        // Imprimir mercancías introducidas por el usuario
    ...
    /// <param name="precioKg"></param>
    public static void B_EscribirConsola(List<Mercancia> mercancias, Decimal precioKg)
    {
        Decimal precioTotal;
        Dictionary<String, Decimal> mercanciasAgrupa = new Dictionary<String, Decimal>();
        int nContenedores;
        ...
        foreach (Mercancia m in mercancias)
        {
            if (mercanciasAgrupa.ContainsKey(m.Destino))
                mercanciasAgrupa[m.Destino] = mercanciasAgrupa[m.Destino] + m.Peso;
            else
                //enviosAgrupados[m.Destino] = m.Peso;
                mercanciasAgrupa.Add(m.Destino, m.Peso);
        }
        ...
        foreach (String dest in mercanciasAgrupa.Keys)
        {
            // decimal pesoTotal = mercanciasAgrupa[dest]; // devuelve valor peso del diccionario mercancia
            nContenedores = (int)Math.Ceiling(mercanciasAgrupa[dest] / Mercancia.PESO_MAX_CONTAINER); // función techo
            precioTotal = nContenedores * PRECIO_CONTAINER + precioKg * mercanciasAgrupa[dest];
            Console.WriteLine(lineaDeDetalle, dest, mercanciasAgrupa[dest], nContenedores, precioTotal);

            //int nContenedores = (int)(pesoTotal / Mercancia.PESOMAX); // división entera
            // si al dividir el pesoTotal entre el pesoMaximo el resto, es decir sobra peso, sumamos un container
            //if (pesoTotal % Mercancia.PESOMAX != 0) nContenedores++;
        }
        Console.WriteLine($"\nPrecio por kilo en el contenedor {precioKg}");
